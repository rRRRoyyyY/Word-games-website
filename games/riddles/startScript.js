...
function updateBackLink(specificId) {
    ...
    if (backBtn) {
        if (specificId) {
            backBtn.href = `riddlesLibrary.html?difficulty=${difficulty}`;
        } else {
            backBtn.href = "riddlesDifficulty.html";
        }
    }
}

function renderItem() {
    if (sneakPeekInterval) clearInterval(sneakPeekInterval); // Kill any active countdown
    ...
    if (specificId) {
        ...
        const availableRiddles = gameData.filter(riddle => getSavedTries('riddle', riddle.id) > 0);
        mode = "start"
        if (availableRiddles.length === 0) {
            ...
            document.getElementById('questionDisplay').innerHTML = `<span class="allLockedMessage">All riddles are locked   👋😂</span>`
        ...
        currentItem = availableRiddles[randomIndex];
    }
    document.getElementById('informStatus').classList.add('hidden');
    triesLeft = getSavedTries('riddle', currentItem.id); 
    solvedCount = parseInt(localStorage.getItem(`solved_riddle_total_${difficulty}`)) || 0;
    ...
    isAlreadySolved = localStorage.getItem(`status_riddle_${currentItem.id}`);
...
function showAnswerGiveUp() {
    if (isAlreadySolved !== "true") {
        let count = parseInt(localStorage.getItem(`riddles_giveups_${difficulty}`)) || 0;
        count++;
        localStorage.setItem(`riddles_giveups_${difficulty}`, count);
...
function showHint(){
    if (isAlreadySolved !== "true") {
        let count = parseInt(localStorage.getItem(`riddles_hints_${difficulty}`)) || 0;
        count++;
        localStorage.setItem(`riddles_hints_${difficulty}`, count);
...
function checkGuess() {
    ...
    if (userInput.includes(correctKey)) {
        ...
        if (!isAlreadySolved) {
            ...
            localStorage.setItem(`solved_riddle_total_${difficulty}`, solvedCount);
            localStorage.setItem(`status_riddle_${currentItem.id}`, "true");
            document.getElementById('informStatus').classList.remove('hidden'); 
...
loadGameData(dataFile).then((data) => {
    ...
    console.log("Total questions loaded:", questionCount); // Debug line
    renderItem();
});