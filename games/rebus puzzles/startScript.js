...
function renderItem() {
    ...
    if (backBtn) {
        if (specificId) {
            backBtn.href = "rebusLibrary.html";
        } else {
            backBtn.href = "rebus.html";
        }
    }

    if (specificId) {
        currentItem = gameData.find(rebus => rebus.id == specificId);
    } else {
    ...
    document.getElementById('questionDisplay').src = currentItem.question;
...
function checkGuess() {
    ...
    let correctKey = currentItem.answer.toLowerCase();
